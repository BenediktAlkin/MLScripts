...
def main(rank):
    ...
    iterate(DataLoader(ds, batch_size=128, sampler=None, drop_last=True))
    dist.barrier()
    if rank == 0: print("sampler=None drop_last=False")
    iterate(DataLoader(ds, batch_size=128, sampler=None, drop_last=False))
    dist.barrier()
    if rank == 0: print("sampler=(drop_last=False) drop_last=False")
    iterate(DataLoader(ds, batch_size=128, sampler=DistributedSampler(ds, drop_last=False), drop_last=False))
    dist.barrier()
    if rank == 0: print("sampler=(drop_last=True) drop_last=False")
    iterate(DataLoader(ds, batch_size=128, sampler=DistributedSampler(ds, drop_last=True), drop_last=False))
    dist.barrier()
    if rank == 0: print("sampler=(drop_last=False) drop_last=True")
    iterate(DataLoader(ds, batch_size=128, sampler=DistributedSampler(ds, drop_last=True), drop_last=True))
    dist.barrier()
